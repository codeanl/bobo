...
func (*User) Login(c *gin.Context, req req.UserLoginReq) (token string, code int) {
	...
	dao.Update(&user)
	// 存入redis
	role := dao.GetOne(model.Role{}, "id", user.RoleID)
	utils.Redis.Set(KEY_USER+loginInfo, utils.Json.Marshal(dto.UserLoginDTO{
		...
		Role:          role,
		Token:         token,
	}), time.Duration(config.Cfg.JWT.Expire*60*60)*time.Second)
	//登陆日志
	dao.Create(&model.LoginLog{
		UserId:    user.ID,
		Type:      1, //用户端
		IpAddress: ipAddress,
		IpSource:  ipSource,
...
func (*User) Profile(userID int) (user resp.UserInfoResp, code int) {
	...
	role := dao.GetOne(model.Role{}, "id", userinfo.RoleID)
	user = utils.CopyProperties[resp.UserInfoResp](userinfo)
	user.Role = role
	//TODO 个人点赞列表
	return user, r.OK
}
...
func (*User) UpdateProfile(req req.UpdateProfileReq, userID int) (code int) {
	...
	dao.Update(&userinfo, "avatar", "nickname")
	return r.OK
}
...
func (*User) UpdateEmail(req req.UpdateEmailReq, userID int) (code int) {
	...
	userSer1.Email = req.Email
	dao.Update(&userSer1)
	return r.OK
}

// Retrieve 找回账户
func (*User) Retrieve(req req.RetrieveReq) (code int) {
	// 检查邮箱是否已存在
	user := dao.GetOne(model.User{}, "email", req.Email)
	if user.ID == 0 {
		return r.ERROR_EMAIL_NOT_EXIST
	}
	// 检查验证码是否正确
	...
	user.Password = utils.Encryptor.BcryptHash(req.Password)
	dao.Update(&user, "password")

	content := fmt.Sprintf(`
		<div>
            	<p>
					您的邮箱 %s 绑定的账户为: %s ，请通过您刚修改的密码 %s 继续登陆。
				</p>
		</div>
	`, req.Email, user.Username, req.Password)
	if err := utils.Email(req.Email, "找回账户", content); err != nil {
		return r.ERROR_EMAIL_SEND
	}
	return r.OK
}
